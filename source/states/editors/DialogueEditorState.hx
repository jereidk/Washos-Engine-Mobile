...
class DialogueEditorState extends MusicBeatState
{
	...
	override function create() {
		...
		#else
		var addLineText:FlxText = new FlxText(10, 10, FlxG.width - 20, 'Press A to remove the current dialogue line, Press P to add another line after the current one.', 8);
        #end
	...
	override function update(elapsed:Float) {
		...
		if(!blockInput) {
			...
			var controlAnim:Array<Bool> = [#if desktop FlxG.keys.justPressed.W #else virtualPad.buttonUp.justPressed #end, #if desktop FlxG.keys.justPressed.S #else virtualPad.buttonDown.justPressed #end];
			var controlText:Array<Bool> = [#if desktop  FlxG.keys.justPressed.D #else virtualPad.buttonLeft.justPressed #end, #if desktop FlxG.keys.justPressed.A #else virtualPad.buttonRight.justPressed #end];
	...
	function saveDialogue() {
		...
		{
			#if android
			StorageSystem.saveContent("dialogue", ".json", data);
			#else
			_file = new FileReference();
