...
class DialogueCharacterEditorState extends MusicBeatState
{
	...
	override function update(elapsed:Float) {
		...
		if(!blockInput && !animationDropDown.dropPanel.visible) {
			...
			if(#if desktop FlxG.keys.justPressed.SPACE #else virtualPad.buttonA.justPressed #end && UI_mainbox.selected_tab_id == 'Character') {
			...
			if(UI_mainbox.selected_tab_id == 'Animations' && curSelectedAnim != null && character.dialogueAnimations.exists(curSelectedAnim)) {
				...
				var animShit:DialogueAnimArray = character.dialogueAnimations.get(curSelectedAnim);
				var controlArrayLoop:Array<Bool> = [#if desktop FlxG.keys.justPressed.A #else virtualPad.buttonLeft.justPressed #end, #if desktop FlxG.keys.justPressed.W #else virtualPad.buttonUp.justPressed #end, #if desktop FlxG.keys.justPressed.D #else virtualPad.buttonRight.justPressed #end, #if desktop FlxG.keys.justPressed.S #else virtualPad.buttonDown.justPressed #end];
				var controlArrayIdle:Array<Bool> = [#if desktop  FlxG.keys.justPressed.LEFT #else virtualPad.buttonLeft.justPressed #end, #if desktop FlxG.keys.justPressed.UP #else virtualPad.buttonUp.justPressed #end, #if desktop FlxG.keys.justPressed.RIGHT #else virtualPad.buttonRight.justPressed #end, #if desktop FlxG.keys.justPressed.DOWN #else virtualPad.buttonDown.justPressed #end];
				#if mobile
	...
	function saveCharacter() {
		...
		if (data.length > 0)
		{
			...
			#if ios
			StorageSystem.saveContent(characterName, ".json", data.trim());
			#else
			_file = new FileReference();
