...
class DialogueCharacterEditorState extends MusicBeatState
{
	...
	function onLoadComplete(_:Event):Void
	{
		...
		if (hasPath)
		{
			rawJson = sys.io.File.getContent(_file.__path);
		}
		...
		if (rawJson != null && rawJson.length > 0)
		{
			try
			{
				var loadedChar:DialogueCharacterFile = cast haxe.Json.parse(rawJson);

				if (loadedChar.dialogue_pos != null)
				{
					var cutName:String = _file.name.substr(0, _file.name.length - 5);
					trace("Successfully loaded character: " + cutName);
			...
			{
				trace("Error Parsing Character JSON: " + e);
			}
		}

		trace("Error: Loading Character Data");
		_file = null;
	}
